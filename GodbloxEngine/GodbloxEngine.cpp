...
HRESULT InitDevice()
{
	...
	/*NOTA: El static_cast<unsigned int> se est� utilizando aqu� para convertir
	el resultado del m�todo size() de un std::vector a un tipo unsigned int.
	El m�todo size() devuelve un valor del tipo std::size_t, que es un tipo
	espec�fico de tama�o no negativo. En algunas plataformas, std::size_t puede
	ser de un tama�o diferente a unsigned int.*/
...
void Update(float DeltaTime) {
	...
	g_World = XMMatrixRotationY(DeltaTime) * XMMatrixRotationX(DeltaTime);
	...
	// Actualizar informacion logica del mesh
